...
def create_review():
    ...
    try:
        ...
        if data.get('wine_id') and data.get('coffee_id'):
            return jsonify({'error': 'Cannot review both wine and coffee in the same review'}), 400
        if not data.get('wine_id') and not data.get('coffee_id'):
...
def get_customer_reviews(customer_id):
    ...
    try:
        ...
        reviews = ReviewModel.get_reviews_by_customer(customer_id, limit)
