...
def get_pairing_recommendations():
    ...
    try:
        ...
        use_frequent = request.args.get('use_frequent', 'true').lower() == 'true'
        
        if wine_id and coffee_id:
            return jsonify({'error': 'Provide either wine_id or coffee_id, not both'}), 400
        elif wine_id:
            # Try frequently bought together first, then fallback to pairings table
            recommendations = []
            if use_frequent:
                try:
                    ...
                    cursor = conn.cursor()
                    cursor.callproc('GetFrequentlyBoughtTogether', (wine_id, None, 5))
                    frequent_results = cursor.fetchall()
                    if frequent_results:
                        recommendations = frequent_results
                    cursor.close()
                    conn.close()
                except Exception as e:
                    print(f"Error getting frequent pairings: {e}")
            
            # Fallback to pairings table if no frequent data
            if not recommendations:
                pairings = PairingModel.get_pairings_by_wine(wine_id)
                recommendations = [p for p in pairings if p.get('pairing_score', 0) >= 7.0]
            ...
            if use_frequent:
                try:
                    ...
                    conn = get_db_connection()
                    cursor = conn.cursor()
                    cursor.callproc('GetFrequentlyBoughtTogether', (None, coffee_id, 5))
                    frequent_results = cursor.fetchall()
                    if frequent_results:
                        recommendations = frequent_results
                    cursor.close()
