...
class ReviewModel:
    ...
    def create_review(review_data):
        ...
        try:
            with conn.cursor() as cursor:
                sql = """INSERT INTO reviews (wine_id, coffee_id, rating, comment, customer_id)
                         VALUES (%s, %s, %s, %s, %s)"""
                cursor.execute(sql, (
                    review_data.get('wine_id'),
                    review_data.get('coffee_id'),
                    review_data['rating'],
    ...
    def get_review_by_id(review_id):
        ...
        try:
            with conn.cursor() as cursor:
                sql = """SELECT r.*, cust.name as customer_name, w.name as wine_name, c.name as coffee_name
                         FROM reviews r
    ...
    def get_reviews_by_wine(wine_id, limit=None):
        ...
        try:
            with conn.cursor() as cursor:
                sql = """SELECT r.*, cust.name as customer_name
                         FROM reviews r
                         LEFT JOIN customers cust ON r.customer_id = cust.id
                         WHERE r.wine_id = %s
    ...
    @staticmethod
    def get_reviews_by_customer(customer_id, limit=None):
        """Get all reviews by a specific customer"""
        conn = get_db_connection()
        try:
            with conn.cursor() as cursor:
                sql = """SELECT r.*, w.name as wine_name, c.name as coffee_name, cust.name as customer_name
                         FROM reviews r
                         LEFT JOIN wines w ON r.wine_id = w.id
                         LEFT JOIN coffees c ON r.coffee_id = c.id
                         LEFT JOIN customers cust ON r.customer_id = cust.id
