...
-- Users table (reintroduced for authentication)
...
-- Basic preferences storage for logged-in experiences
CREATE TABLE IF NOT EXISTS user_preferences (
    id INT AUTO_INCREMENT PRIMARY KEY,
    user_id INT NOT NULL,
    preference_type ENUM('wine', 'coffee') NOT NULL,
    preference_key VARCHAR(100) NOT NULL,
    preference_value VARCHAR(255) NOT NULL,
    UNIQUE KEY uniq_user_pref (user_id, preference_type, preference_key),
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
);

-- Seed default admin account (username: inesh, password: 12345)
...
-- Payments table (optional - payment_mode is nullable, records may not be created)
CREATE TABLE IF NOT EXISTS payments (
    id INT AUTO_INCREMENT PRIMARY KEY,
    order_id INT NOT NULL,
    payment_mode ENUM('cash','card','upi','netbanking') NULL,
...
-- Saved payment and fulfilment profiles per authenticated user
CREATE TABLE IF NOT EXISTS customer_payment_profiles (
    ...
    country VARCHAR(120),
    payment_method ENUM('upi','cod','card','netbanking') NULL,
    upi_id VARCHAR(120),
    delivery_instructions TEXT,
...
-- Table to track how often products are bought together
CREATE TABLE IF NOT EXISTS frequently_bought_together (
    id INT AUTO_INCREMENT PRIMARY KEY,
    wine_id INT NULL,
    coffee_id INT NULL,
    paired_wine_id INT NULL,
    paired_coffee_id INT NULL,
    purchase_count INT DEFAULT 1,
    last_purchased TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY unique_pairing (wine_id, coffee_id, paired_wine_id, paired_coffee_id),
    FOREIGN KEY (wine_id) REFERENCES wines(id) ON DELETE CASCADE,
    FOREIGN KEY (coffee_id) REFERENCES coffees(id) ON DELETE CASCADE,
    FOREIGN KEY (paired_wine_id) REFERENCES wines(id) ON DELETE CASCADE,
    FOREIGN KEY (paired_coffee_id) REFERENCES coffees(id) ON DELETE CASCADE,
    CHECK (
        (wine_id IS NOT NULL AND paired_coffee_id IS NOT NULL) OR
        (coffee_id IS NOT NULL AND paired_wine_id IS NOT NULL)
    )
);

CREATE INDEX IF NOT EXISTS idx_fbt_wine ON frequently_bought_together(wine_id);
CREATE INDEX IF NOT EXISTS idx_fbt_coffee ON frequently_bought_together(coffee_id);